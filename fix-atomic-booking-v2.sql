-- Corrigir função de agendamento atômico v2
-- Remove FOR UPDATE com COUNT() que não é permitido
-- Usa EXISTS ao invés de COUNT para verificação de conflitos
...
BEGIN
  ...
  -- 2. Verificar conflitos de agendamento usando EXISTS (mais eficiente)
  SELECT EXISTS (
    SELECT 1
    FROM bookings
    WHERE (
...
EXCEPTION
  WHEN unique_violation THEN
    -- Tratar violação de chave única (race condition)
    RAISE EXCEPTION 'BOOKING_CONFLICT: Booking slot already taken';
