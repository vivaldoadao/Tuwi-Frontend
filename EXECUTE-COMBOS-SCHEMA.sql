...
-- 1. Create promotion_combos table
...
-- 2. Create promotion_subscriptions table
...
-- 3. Create promotion_subscription_executions table
...
-- 4. Create promotion_coupons table
...
-- 5. Create coupon usage tracking table
...
-- 6. Create promotion_combo_notifications table  
...
CREATE POLICY "Only admins can manage combos" ON public.promotion_combos
  FOR ALL USING (
...
-- Coupons: Public read (active ones), admin write
...
CREATE POLICY "Only admins can manage coupons" ON public.promotion_coupons
  FOR ALL USING (
    auth.jwt() ->> 'role' = 'admin' OR 
    auth.jwt() ->> 'user_metadata' ->> 'role' = 'admin'
  );

-- Coupon usage: Users see their own usage, admins see all
...
CREATE POLICY "System can create notifications" ON public.promotion_combo_notifications
  FOR INSERT WITH CHECK (true); -- System service can create for any user
